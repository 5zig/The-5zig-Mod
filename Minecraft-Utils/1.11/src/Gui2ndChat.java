...
//		((Variables) MinecraftFactory.getVars()).getGuiIngame().d().a(component);
	...
	private void setChatLine(fb chatComponent, int id, int currentUpdateCounter, boolean refresh) {
		...
		List<fb> lines = bfm.a(chatComponent, lineWidth, ((Variables) MinecraftFactory.getVars()).getFontrenderer(), false, false);
	...
	public void drawComponentHover(int mouseX, int mouseY) {
		...
		if (chatComponent != null && chatComponent.b().i() != null)
			((bhm) MinecraftFactory.getVars().getMinecraftScreen()).a(chatComponent, mouseX, mouseY);
	}
	...
	private fb getChatComponent(int mouseX, int mouseY) {
		if (!this.isChatOpened()) {
			...
			bfi scaledResolution = new bfi(((Variables) MinecraftFactory.getVars()).getMinecraft());
	...
	private String optStripColor(String text) {
		return bfm.a(text, false);
	}

	public boolean isChatOpened() {
		return MinecraftFactory.getVars().getMinecraftScreen() instanceof bgp;
	}
