...
public class ResourceManager implements IResourceManager {
	...
	public ResourceManager(GameProfile playerProfile) throws ClassNotFoundException, NoSuchFieldException, IllegalAccessException {
		guiCameraTransform = Thread.currentThread().getContextClassLoader().loadClass(brq.class.getName() + (Transformer.FORGE ? "$TransformType" : "$b")).getDeclaredField(
				...
				try {
					...
					if (data.models != null) {
						...
						for (TextureData.Model model : models) {
							...
							try {
								afh item = afh.g.c(new ResourceLocation(model.itemName));
								if (item != null) {
	...
	private cbh createModel(String model) {
		bri modelBlock = bri.a(model);

		return bakeModel(modelBlock);
	}

	private cbh bakeModel(bri modelBlock) {
		cbo.a bakedModelBuilder = (new cbo.a(modelBlock, modelBlock.g())).a(TextureAtlasSprite.MISSING_NO);

		for (bre blockPart : modelBlock.a()) {
			for (cv face : blockPart.c.keySet()) {
				brf blockPartFace = blockPart.c.get(face);
				if (blockPartFace.b == null) {
					bakedModelBuilder.a(faceBakery.a(blockPart.a, blockPart.b, blockPartFace, face, cbi.a, blockPart.d, false, blockPart.e));
				} else {
					bakedModelBuilder.a(cbi.a.a(blockPartFace.b), faceBakery.a(blockPart.a, blockPart.b, blockPartFace, face, cbi.a, blockPart.d, false, blockPart.e));
				}
			}
		}
	...
	private boolean shouldRender(ItemModelResource itemModelResource, aay entityPlayer, afj itemStack) {
		if (itemStack.c() == itemModelResource.getItem()) {
			if (itemStack.c() == afl.g && entityPlayer.bV() != null) {
				// bow
			...
			} else if (itemStack.c() == afl.aZ && entityPlayer.bQ != null) {
				// fishing rod
	...
	public boolean renderInPersonMode(Object instance, Object itemStackObject, Object entityPlayerObject, Object cameraTransformTypeObject, boolean leftHand) {
		...
		afj itemStack = (afj) itemStackObject;
		if (!(entityPlayerObject instanceof aay)) {
			return false;
		}
		aay entityPlayer = (aay) entityPlayerObject;
		brq.b cameraTransformType = (brq.b) cameraTransformTypeObject;
	...
	private void render(Object instance, afj itemStack, ItemModelResource itemModel, brq.b transformType, boolean leftHand) {
		...
		bqg.a(516, 0.1F);
		bqg.m();
		bqg.a(bqg.r.l, bqg.l.j, bqg.r.e, bqg.l.n);
		bqg.G();
		brq itemCameraTransforms = ((cbh) itemModel.getBakedModel()).e();
		brq.a(itemCameraTransforms.b(transformType), leftHand);
		if (a(itemCameraTransforms.b(transformType))) {
			bqg.a(bqg.i.a);
		}

		((bvh) instance).a(itemStack, (cbh) itemModel.getBakedModel());
		bqg.a(bqg.i.b);
		bqg.H();
		bqg.E();
		bqg.l();
	...
	public boolean renderInInventory(Object instance, Object itemStackObject, int x, int y) {
		...
		for (ItemModelResource itemModel : ownPlayerResource.getItemModelResources()) {
			bps player = ((Variables) MinecraftFactory.getVars()).getPlayer();
			if (shouldRender(itemModel, player, itemStack) && (itemModel.getRender() == null || player.bs.i() == itemStack)) {
				...
				bqg.a(516, 0.1F);
				bqg.m();
				bqg.a(bqg.r.l, bqg.l.j);
				bqg.c(1.0F, 1.0F, 1.0F, 1.0F);
				a((bvh) instance, x, y, ((cbh) itemModel.getBakedModel()).b());
				((cbh) itemModel.getBakedModel()).e().a((brq.b) guiCameraTransform);
				((bvh) instance).a(itemStack, (cbh) itemModel.getBakedModel());
				bqg.d();
				bqg.E();
				bqg.g();
	...
	private void a(bvh instance, int var, int var1, boolean var2) {
		bqg.c((float) var, (float) var1, 100.0F + instance.a);
