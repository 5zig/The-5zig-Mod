...
public class WrappedItemStack implements ItemStack {
	...
	public WrappedItemStack(afj item) {
		this.item = item;
	}
	...
	public int getHealAmount() {
		return item.c() instanceof afd ? ((afd) item.c()).h(item) : 0;
	}

	@Override
	public float getSaturationModifier() {
		return item.c() instanceof afd ? ((afd) item.c()).i(item) : 0;
	}
	...
	public void render(int x, int y, boolean withGenericAttributes) {
		...
		if (withGenericAttributes) {
			for (ss modifier : ss.values()) {
				Multimap<String, tl> multimap = item.a(modifier);
				for (Map.Entry<String, tl> entry : multimap.entries()) {
					tl attribute = entry.getValue();
					double value = attribute.d();
					if (ITEM_MODIFIER_UUID.equals(attribute.a())) {
						if (((Variables) MinecraftFactory.getVars()).getPlayer() == null) {
							value += 1;
							value += (double) aik.a(item, tb.a);
						} else {
							value += ((Variables) MinecraftFactory.getVars()).getPlayer().a(aad.e).b();
							value += (double) aik.a(item, tb.a);
						}
					}
	...
	public static String getResourceKey(afj item) {
		kq resourceLocation = afh.g.b(item.c());
