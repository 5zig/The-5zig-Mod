...
public class WrappedItemStack implements ItemStack {
	...
	public WrappedItemStack(amj item) {
		this.item = item;
	}
	...
	public int getHealAmount() {
		return item.b() instanceof all ? ((all) item.b()).h(item) : 0;
	}

	@Override
	public float getSaturationModifier() {
		return item.b() instanceof all ? ((all) item.b()).i(item) : 0;
	}
	...
	public void render(int x, int y, boolean withGenericAttributes) {
		...
		if (withGenericAttributes) {
			Multimap<String, ya> multimap = item.B();
			for (Map.Entry<String, ya> entry : multimap.entries()) {
				ya attribute = entry.getValue();
				double value = attribute.d();
				if (nameDisplayModifier.equals(attribute.a())) {
					value = value + (double) aph.a(item, xs.a);
				}
	...
	public static String getResourceKey(amj item) {
		Object resourceLocation = alq.e.c(item.b());
