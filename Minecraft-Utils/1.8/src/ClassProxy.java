...
public class ClassProxy {
	...
	static {
		...
		try {
			...
			Class<?> aClass = classLoader.loadClass(bzu.class.getName());
			if (Transformer.FORGE) {
	...
	public static void guiOptionsActionPerformed(Object instance, Object b) {
		if (((bug) b).k == 42) {
			MinecraftFactory.getClassProxyCallback().displayGuiSettings(new WrappedGui((bxf) instance));
		}
	}

	public static IButton getMCPVPButton(bxf guiScreen) {
	...
	public static void guiMultiplayerActionPerformed(bxf guiScreen, bug button) {
		if (button.k == 9) {
//			The5zigMod.getVars().displayScreen(new ServerListMCPvP(guiScreen));
	...
	public static void handleGuiResourcePackInit(Object instance, Object listObject, Object listObject2) {
		...
		bzx gui = (bzx) instance;
		Comparator<bzy> comparator = new Comparator<bzy>() {
			@Override
			public int compare(bzy o1, bzy o2) {
				if (!(o1 instanceof caa) || !(o2 instanceof caa))
					return 0;
				caa resourcePackListEntryFound1 = (caa) o1;
				caa resourcePackListEntryFound2 = (caa) o2;
		...
		MinecraftFactory.getClassProxyCallback().addSearch(
				new SearchEntry<bzy>(MinecraftFactory.getVars().createTextfield(MinecraftFactory.getClassProxyCallback().translate("gui.search"), 9991, gui.l / 2 - 200, gui.m - 70, 170, 16),
						list, comparator) {
					@Override
					public boolean filter(String text, bzy o) {
						if (!(o instanceof caa))
							return true;
						caa resourcePackListEntryFound = (caa) o;
				...
				new SearchEntry<bzy>(MinecraftFactory.getVars().createTextfield(MinecraftFactory.getClassProxyCallback().translate("gui.search"), 9992, gui.l / 2 + 8, gui.m - 70, 170, 16),
						list2, new Comparator<bzy>() {
					@Override
					public int compare(bzy o1, bzy o2) {
						return 0;
					}
	...
	public static void handleGuiMultiplayerInit(Object instance, Object serverSelectionListInstance) {
		final bzp guiMultiplayer = (bzp) instance;
		final bzu serverSelectionList = (bzu) serverSelectionListInstance;
		final List<bzs> list;
		try {
			list = (List<bzs>) ClassProxy.serverList.get(serverSelectionList);
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
		...
		final SearchEntry<bzs> searchEntry = new SearchEntry<bzs>(textfield, list) {
			...
			@Override
			public boolean filter(String text, bzs serverListEntry) {
		...
		Callback<bzs> enterCallback = new Callback<bzs>() {
			@Override
			public void call(bzs callback) {
				guiMultiplayer.b(0);
				guiMultiplayer.f();
				searchEntry.reset();
			}
	...
	public static void handleGuiSelectWorldInit(Object instance, List list) {
		final bxg guiSelectWorld = (bxg) instance;
		...
		final SearchEntry<brb> searchEntry = new SearchEntry<brb>(textfield, list) {
			@Override
			public boolean filter(String text, brb saveFormatComparator) {
		...
		searchEntry.setEnterCallback(new Callback<brb>() {
			@Override
			public void call(brb callback) {
				guiSelectWorld.i(0);
			}
		});
		searchEntry.setComparator(new Comparator<brb>() {
			@Override
			public int compare(brb saveFormatComparator1, brb saveFormatComparator2) {
	...
	public static void renderSnow() {
		if (MinecraftFactory.getVars().getMinecraftScreen() instanceof bxq) {
			MinecraftFactory.getClassProxyCallback().renderSnow(((bxq) MinecraftFactory.getVars().getMinecraftScreen()).l, ((bxq) MinecraftFactory.getVars().getMinecraftScreen()).m);
		}
	}

	public static float getCustomFOVModifier(Object object) {
		cil playerInstance = (cil) object;

		float modifier = 1.0F;
		if (playerInstance.by.b) {
			// is Flying
			modifier *= 1.1F;
		}
		xz movementSpeedAttribute = playerInstance.a(afs.d);
		if (movementSpeedAttribute.a(SPRINT_MODIFIER_UUID) != null) {
			modifier = (float) ((double) modifier * (movementSpeedAttribute.b() * 1.30000001192092896D / playerInstance.by.b() + 1.0) / 2.0D);
		}
		if (playerInstance.by.b() == 0.0F || Float.isNaN(modifier) || Float.isInfinite(modifier)) {
			modifier = 1.0F;
		}

		if (playerInstance.bR() && playerInstance.bP().b() == amk.f) {
			// is using bow
			int itemInUseDuration = playerInstance.bS();
	...
	public static void setSignText(Object signTile, String[] text) {
		for (int i = 0; i < text.length; i++) {
			ho[] components = ((bdj) signTile).a;
			if (!Objects.equal(components[i].d(), text[i])) {
				components[i] = new hy(text[i]);
			}
		}
	}
