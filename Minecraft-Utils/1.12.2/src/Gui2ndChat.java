...
//		((Variables) MinecraftFactory.getVars()).getGuiIngame().d().a(component);
		...
		if (MinecraftFactory.getClassProxyCallback().is2ndChatVisible()) {
			...
			if (chatLineCount > 0) {
				...
				int width = rk.f((float) this.getChatWidth() / chatScale);
				GLUtil.pushMatrix();
				if (Transformer.FORGE) {
						...
						if (ticksPassed < 200 || chatOpened) {
							...
							if (alpha > 3) {
								...
								if (!MinecraftFactory.getClassProxyCallback().isChatBackgroundTransparent()) {
									Gui.drawRect(x, y - 9, x + width + rk.d(4f * chatScale), y, alpha / 2 << 24);
								}
	...
	private void setChatLine(hh chatComponent, int id, int currentUpdateCounter, boolean refresh) {
		...
		int lineWidth = rk.d((float) this.getChatWidth() / this.getChatScale());
		List<hh> lines = bjc.a(chatComponent, lineWidth, ((Variables) MinecraftFactory.getVars()).getFontrenderer(), false, false);
	...
	public void drawComponentHover(int mouseX, int mouseY) {
		...
		if (chatComponent != null && chatComponent.b().i() != null)
			((blk) MinecraftFactory.getVars().getMinecraftScreen()).a(chatComponent, mouseX, mouseY);
	}
	...
	public boolean mouseClicked(int mouseX, int mouseY, int button) {
		try {
			return button == 0 && (clickChatComponent != null ? (Boolean) clickChatComponent.invoke(MinecraftFactory.getVars().getMinecraftScreen(),
					getChatComponent(Mouse.getX(), Mouse.getY())) : ((bkn) MinecraftFactory.getVars().getMinecraftScreen()).a(getChatComponent(Mouse.getX(), Mouse.getY())));
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
	...
	private hh getChatComponent(int mouseX, int mouseY) {
		if (!this.isChatOpened()) {
			...
			bit scaledResolution = new bit(((Variables) MinecraftFactory.getVars()).getMinecraft());
			...
			x = rk.d((float) x / chatScale);
			y = rk.d((float) y / chatScale);
			if (x >= 0 && y >= 0) {
				int lineCount = Math.min(this.getLineCount(), this.singleChatLines.size());
				if (x <= rk.d((float) this.getChatWidth() / this.getChatScale() + 3 / getChatScale()) && y < MinecraftFactory.getVars().getFontHeight() * lineCount) {
	...
	private String optStripColor(String text) {
		return bjc.a(text, false);
	}

	public boolean isChatOpened() {
		return MinecraftFactory.getVars().getMinecraftScreen() instanceof bkn;
	}
	...
	public int getChatWidth() {
		return rk.d(MinecraftFactory.getClassProxyCallback().get2ndChatWidth());
	}

	public int getChatHeight() {
		return rk.d(this.isChatOpened() ? MinecraftFactory.getClassProxyCallback().get2ndChatHeightFocused() : MinecraftFactory.getClassProxyCallback().get2ndChatHeightUnfocused());
	}
