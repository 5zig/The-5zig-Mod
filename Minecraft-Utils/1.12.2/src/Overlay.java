...
public class Overlay implements IOverlay {
	...
	private static class Toast implements bkb {

		private static final bkb.a ENUM_SHOW, ENUM_HIDE;

		static {
			try {
				Class<?> enumClass = Thread.currentThread().getContextClassLoader().loadClass("bkb" + "$a");
				ENUM_SHOW = (bkb.a) enumClass.getField(Transformer.FORGE ? "SHOW" : "a").get(null);
				ENUM_HIDE = (bkb.a) enumClass.getField(Transformer.FORGE ? "HIDE" : "b").get(null);
			} catch (Exception e) {
		...
		@Override
		public bkb.a a(bkc var1, long l) {
			if (updateTime) {
				startedTime = l;
				updateTime = false;
			}
		...
		public static void create(String title, String subTitle, Object uniqueReference) {
			bkc toastFactory = ((Variables) MinecraftFactory.getVars()).getMinecraft().ao();
