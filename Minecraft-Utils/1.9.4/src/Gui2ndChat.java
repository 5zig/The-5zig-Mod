...
//		ew component = new fc("Test");
//		ez style = new ez();
//		style.a(new ew(ew.a.a, new fc("Hover!")));
...
//		((Variables) MinecraftFactory.getVars()).getGuiIngame().d().a(component);
		...
		if (MinecraftFactory.getClassProxyCallback().is2ndChatVisible()) {
			...
			if (chatLineCount > 0) {
				...
				int width = on.f((float) this.getChatWidth() / chatScale);
				GLUtil.pushMatrix();
				if (Transformer.FORGE) {
						...
						if (ticksPassed < 200 || chatOpened) {
							...
							if (alpha > 3) {
								...
								if (!MinecraftFactory.getClassProxyCallback().isChatBackgroundTransparent()) {
									Gui.drawRect(x, y - 9, x + width + on.d(4f * chatScale), y, alpha / 2 << 24);
								}
	...
	public void printChatMessage(Object chatComponent) {
		if (!(chatComponent instanceof ew))
			throw new IllegalArgumentException(chatComponent.getClass().getName() + " != " + ew.class.getName());
		printChatMessage((ew) chatComponent);
	}

	public void printChatMessage(ew chatComponent) {
		this.printChatMessage(chatComponent, 0);
	}

	public void printChatMessage(ew chatComponent, int id) {
	...
	private void setChatLine(ew chatComponent, int id, int currentUpdateCounter, boolean refresh) {
		if (!refresh && MinecraftFactory.getClassProxyCallback().isShowTimeBeforeChatMessage()) {
			chatComponent = (ew) MinecraftFactory.getClassProxyCallback().getChatComponentWithTime(chatComponent);
		}
		if (id != 0) {
			this.deleteChatLine(id);
		}

		int lineWidth = on.d((float) this.getChatWidth() / this.getChatScale());
		List<ew> lines = bcz.a(chatComponent, lineWidth, ((Variables) MinecraftFactory.getVars()).getFontrenderer(), false, false);
		boolean var6 = this.isChatOpened();

		ew lineString;
		for (Iterator<ew> iterator = lines.iterator(); iterator.hasNext(); this.singleChatLines.add(0, new GuiChatLine(currentUpdateCounter, lineString, id))) {
	...
	public void drawComponentHover(int mouseX, int mouseY) {
		ew chatComponent = getChatComponent(Mouse.getX(), Mouse.getY());
		if (chatComponent != null && chatComponent.b().i() != null)
			((bey) MinecraftFactory.getVars().getMinecraftScreen()).a(chatComponent, mouseX, mouseY);
	}
	...
	public boolean mouseClicked(int mouseX, int mouseY, int button) {
		try {
			return button == 0 && (clickChatComponent != null ? (Boolean) clickChatComponent.invoke(MinecraftFactory.getVars().getMinecraftScreen(),
					getChatComponent(Mouse.getX(), Mouse.getY())) : ((bec) MinecraftFactory.getVars().getMinecraftScreen()).a(getChatComponent(Mouse.getX(), Mouse.getY())));
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
	...
	private ew getChatComponent(int mouseX, int mouseY) {
		if (!this.isChatOpened()) {
			return null;
		} else {
			bcv scaledResolution = new bcv(((Variables) MinecraftFactory.getVars()).getMinecraft());
			...
			x = on.d((float) x / chatScale);
			y = on.d((float) y / chatScale);
			if (x >= 0 && y >= 0) {
				int lineCount = Math.min(this.getLineCount(), this.singleChatLines.size());
				if (x <= on.d((float) this.getChatWidth() / this.getChatScale() + 3 / getChatScale()) && y < MinecraftFactory.getVars().getFontHeight() * lineCount) {
					...
					if (lineId >= 0 && lineId < this.singleChatLines.size()) {
						...
						for (ew chatComponent : chatLine.getChatComponent()) {
							if (chatComponent instanceof fc) { // ChatComponentText
								widthCounter += MinecraftFactory.getVars().getStringWidth(optStripColor(((fc) chatComponent).g()));
								if (widthCounter > x) {
	...
	private String optStripColor(String text) {
		return bcz.a(text, false);
	}

	public boolean isChatOpened() {
		return MinecraftFactory.getVars().getMinecraftScreen() instanceof bec;
	}
	...
	public int getChatHeight() {
		return on.d(this.isChatOpened() ? MinecraftFactory.getClassProxyCallback().get2ndChatHeightFocused() : MinecraftFactory.getClassProxyCallback().get2ndChatHeightUnfocused());
	}
	...
	public static void highlightChatLine(ew chatComponent, int x, int y, int alpha) {
		...
		String builder = "";
		for (ew textComponent : chatComponent) {
