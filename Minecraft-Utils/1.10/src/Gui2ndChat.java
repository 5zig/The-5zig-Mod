...
//		ey component = new fe("Test");
//		ez style = new ez();
//		style.a(new ey(ey.a.a, new fe("Hover!")));
...
//		((Variables) MinecraftFactory.getVars()).getGuiIngame().d().a(component);
		...
		if (MinecraftFactory.getClassProxyCallback().is2ndChatVisible()) {
			...
			if (chatLineCount > 0) {
				...
				int width = op.f((float) this.getChatWidth() / chatScale);
				GLUtil.pushMatrix();
				if (Transformer.FORGE) {
						...
						if (ticksPassed < 200 || chatOpened) {
							...
							if (alpha > 3) {
								...
								if (!MinecraftFactory.getClassProxyCallback().isChatBackgroundTransparent()) {
									Gui.drawRect(x, y - 9, x + width + op.d(4f * chatScale), y, alpha / 2 << 24);
								}
	...
	public void printChatMessage(Object chatComponent) {
		if (!(chatComponent instanceof ey))
			throw new IllegalArgumentException(chatComponent.getClass().getName() + " != " + ey.class.getName());
		printChatMessage((ey) chatComponent);
	}

	public void printChatMessage(ey chatComponent) {
		this.printChatMessage(chatComponent, 0);
	}

	public void printChatMessage(ey chatComponent, int id) {
	...
	private void setChatLine(ey chatComponent, int id, int currentUpdateCounter, boolean refresh) {
		if (!refresh && MinecraftFactory.getClassProxyCallback().isShowTimeBeforeChatMessage()) {
			chatComponent = (ey) MinecraftFactory.getClassProxyCallback().getChatComponentWithTime(chatComponent);
		}
		if (id != 0) {
			this.deleteChatLine(id);
		}

		int lineWidth = op.d((float) this.getChatWidth() / this.getChatScale());
		List<ey> lines = bdt.a(chatComponent, lineWidth, ((Variables) MinecraftFactory.getVars()).getFontrenderer(), false, false);
		boolean var6 = this.isChatOpened();

		ey lineString;
		for (Iterator<ey> iterator = lines.iterator(); iterator.hasNext(); this.singleChatLines.add(0, new GuiChatLine(currentUpdateCounter, lineString, id))) {
	...
	public void drawComponentHover(int mouseX, int mouseY) {
		ey chatComponent = getChatComponent(Mouse.getX(), Mouse.getY());
		if (chatComponent != null && chatComponent.b().i() != null)
			((bft) MinecraftFactory.getVars().getMinecraftScreen()).a(chatComponent, mouseX, mouseY);
	}
	...
	private ey getChatComponent(int mouseX, int mouseY) {
		if (!this.isChatOpened()) {
			return null;
		} else {
			bdp scaledResolution = new bdp(((Variables) MinecraftFactory.getVars()).getMinecraft());
			...
			x = op.d((float) x / chatScale);
			y = op.d((float) y / chatScale);
			if (x >= 0 && y >= 0) {
				int lineCount = Math.min(this.getLineCount(), this.singleChatLines.size());
				if (x <= op.d((float) this.getChatWidth() / this.getChatScale() + 3 / getChatScale()) && y < MinecraftFactory.getVars().getFontHeight() * lineCount) {
					...
					if (lineId >= 0 && lineId < this.singleChatLines.size()) {
						...
						for (ey chatComponent : chatLine.getChatComponent()) {
							if (chatComponent instanceof fe) { // ChatComponentText
								widthCounter += MinecraftFactory.getVars().getStringWidth(optStripColor(((fe) chatComponent).g()));
								if (widthCounter > x) {
	...
	private String optStripColor(String text) {
		return bdt.a(text, false);
	}

	public boolean isChatOpened() {
		return MinecraftFactory.getVars().getMinecraftScreen() instanceof bew;
	}
	...
	public int getChatWidth() {
		return op.d(MinecraftFactory.getClassProxyCallback().get2ndChatWidth());
	}

	public int getChatHeight() {
		return op.d(this.isChatOpened() ? MinecraftFactory.getClassProxyCallback().get2ndChatHeightFocused() : MinecraftFactory.getClassProxyCallback().get2ndChatHeightUnfocused());
	}
	...
	public static void highlightChatLine(ey chatComponent, int x, int y, int alpha) {
		...
		String builder = "";
		for (ey textComponent : chatComponent) {
