...
public class ResourceManager implements IResourceManager {
	...
	public ResourceManager(GameProfile playerProfile) throws ClassNotFoundException, NoSuchFieldException, IllegalAccessException {
		guiCameraTransform = Thread.currentThread().getContextClassLoader().loadClass(bwa.class.getName() + (Transformer.FORGE ? "$TransformType" : "$b")).getDeclaredField(
	...
	private PlayerResource loadPlayerResource(final GameProfile gameProfile) {
		...
		cdq texture = ((Variables) MinecraftFactory.getVars()).getTextureManager().b(capeLocation);
				...
				try {
					...
					if (data.models != null) {
						...
						for (TextureData.Model model : models) {
							...
							try {
								ail item = ail.g.c(new ResourceLocation(model.itemName));
								if (item != null) {
	...
	public Object getCapeLocation(Object player) {
		GameProfile profile = ((bty) player).da();
	...
	private cfw createModel(String model) {
		bvs modelBlock = bvs.a(model);

		return bakeModel(modelBlock);
	}

	private cfw bakeModel(bvs modelBlock) {
		cgd.a bakedModelBuilder = (new cgd.a(modelBlock, modelBlock.g())).a(TextureAtlasSprite.MISSING_NO);

		for (bvo blockPart : modelBlock.a()) {
			for (fa face : blockPart.c.keySet()) {
				bvp blockPartFace = blockPart.c.get(face);
				if (blockPartFace.b == null) {
					bakedModelBuilder.a(faceBakery.a(blockPart.a, blockPart.b, blockPartFace, face, cfx.a, blockPart.d, false, blockPart.e));
				} else {
					bakedModelBuilder.a(cfx.a.a(blockPartFace.b), faceBakery.a(blockPart.a, blockPart.b, blockPartFace, face, cfx.a, blockPart.d, false, blockPart.e));
				}
			}
		}
	...
	private boolean shouldRender(ItemModelResource itemModelResource, aeb entityPlayer, ain itemStack) {
		if (itemStack.c() == itemModelResource.getItem()) {
			if (itemStack.c() == aip.g && entityPlayer.ce() != null) {
				// bow
			...
			} else if (itemStack.c() == aip.aZ && entityPlayer.bU != null) {
				// fishing rod
	...
	public boolean renderInPersonMode(Object instance, Object itemStackObject, Object entityPlayerObject, Object cameraTransformTypeObject, boolean leftHand) {
		...
		ain itemStack = (ain) itemStackObject;
		if (!(entityPlayerObject instanceof aeb)) {
			return false;
		}
		aeb entityPlayer = (aeb) entityPlayerObject;
		bwa.b cameraTransformType = (bwa.b) cameraTransformTypeObject;
	...
	private void render(Object instance, ain itemStack, ItemModelResource itemModel, bwa.b transformType, boolean leftHand) {
		...
		buq.G();
		bwa itemCameraTransforms = ((cfw) itemModel.getBakedModel()).e();
		bwa.a(itemCameraTransforms.b(transformType), leftHand);
		if (a(itemCameraTransforms.b(transformType))) {
			buq.a(buq.i.a);
		}

		((bzu) instance).a(itemStack, (cfw) itemModel.getBakedModel());
		buq.a(buq.i.b);
		buq.H();
		buq.E();
		buq.l();
	...
	public boolean renderInInventory(Object instance, Object itemStackObject, int x, int y) {
		...
		for (ItemModelResource itemModel : ownPlayerResource.getItemModelResources()) {
			bub player = ((Variables) MinecraftFactory.getVars()).getPlayer();
			if (shouldRender(itemModel, player, itemStack) && (itemModel.getRender() == null || player.bv.i() == itemStack)) {
				...
				a((bzu) instance, x, y, ((cfw) itemModel.getBakedModel()).b());
				((cfw) itemModel.getBakedModel()).e().a((bwa.b) guiCameraTransform);
				((bzu) instance).a(itemStack, (cfw) itemModel.getBakedModel());
				buq.d();
				buq.E();
				buq.g();
	...
	private void a(bzu instance, int var, int var1, boolean var2) {
		buq.c((float) var, (float) var1, 100.0F + instance.a);
		buq.c(8.0F, 8.0F, 0.0F);
