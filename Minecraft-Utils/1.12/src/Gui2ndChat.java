...
//		hh component = new ho("Test");
//		ez style = new ez();
//		style.a(new hh(hh.a.a, new ho("Hover!")));
//		style.a(new gy(gy.a.a, "http://google.de"));
//		component.a(style);
...
//		((Variables) MinecraftFactory.getVars()).getGuiIngame().d().a(component);
		...
		if (MinecraftFactory.getClassProxyCallback().is2ndChatVisible()) {
			...
			if (chatLineCount > 0) {
				...
				int width = ri.f((float) this.getChatWidth() / chatScale);
				GLUtil.pushMatrix();
				if (Transformer.FORGE) {
						...
						if (ticksPassed < 200 || chatOpened) {
							...
							if (alpha > 3) {
								...
								if (!MinecraftFactory.getClassProxyCallback().isChatBackgroundTransparent()) {
									Gui.drawRect(x, y - 9, x + width + ri.d(4f * chatScale), y, alpha / 2 << 24);
								}
	...
	public void printChatMessage(Object chatComponent) {
		if (!(chatComponent instanceof hh))
			throw new IllegalArgumentException(chatComponent.getClass().getName() + " != " + hh.class.getName());
		printChatMessage((hh) chatComponent);
	}

	public void printChatMessage(hh chatComponent) {
		this.printChatMessage(chatComponent, 0);
	}

	public void printChatMessage(hh chatComponent, int id) {
	...
	private void setChatLine(hh chatComponent, int id, int currentUpdateCounter, boolean refresh) {
		if (!refresh && MinecraftFactory.getClassProxyCallback().isShowTimeBeforeChatMessage()) {
			chatComponent = (hh) MinecraftFactory.getClassProxyCallback().getChatComponentWithTime(chatComponent);
		}
		if (id != 0) {
			this.deleteChatLine(id);
		}

		int lineWidth = ri.d((float) this.getChatWidth() / this.getChatScale());
		List<hh> lines = bja.a(chatComponent, lineWidth, ((Variables) MinecraftFactory.getVars()).getFontrenderer(), false, false);
		boolean var6 = this.isChatOpened();

		hh lineString;
		for (Iterator<hh> iterator = lines.iterator(); iterator.hasNext(); this.singleChatLines.add(0, new GuiChatLine(currentUpdateCounter, lineString, id))) {
	...
	public void drawComponentHover(int mouseX, int mouseY) {
		hh chatComponent = getChatComponent(Mouse.getX(), Mouse.getY());
		if (chatComponent != null && chatComponent.b().i() != null)
			((bli) MinecraftFactory.getVars().getMinecraftScreen()).a(chatComponent, mouseX, mouseY);
	}
	...
	public boolean mouseClicked(int mouseX, int mouseY, int button) {
		try {
			return button == 0 && (clickChatComponent != null ? (Boolean) clickChatComponent.invoke(MinecraftFactory.getVars().getMinecraftScreen(),
					getChatComponent(Mouse.getX(), Mouse.getY())) : ((bkl) MinecraftFactory.getVars().getMinecraftScreen()).a(getChatComponent(Mouse.getX(), Mouse.getY())));
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
	...
	private hh getChatComponent(int mouseX, int mouseY) {
		if (!this.isChatOpened()) {
			return null;
		} else {
			bir scaledResolution = new bir(((Variables) MinecraftFactory.getVars()).getMinecraft());
			...
			x = ri.d((float) x / chatScale);
			y = ri.d((float) y / chatScale);
			if (x >= 0 && y >= 0) {
				int lineCount = Math.min(this.getLineCount(), this.singleChatLines.size());
				if (x <= ri.d((float) this.getChatWidth() / this.getChatScale() + 3 / getChatScale()) && y < MinecraftFactory.getVars().getFontHeight() * lineCount) {
					...
					if (lineId >= 0 && lineId < this.singleChatLines.size()) {
						...
						for (hh chatComponent : chatLine.getChatComponent()) {
							if (chatComponent instanceof ho) { // ChatComponentText
								widthCounter += MinecraftFactory.getVars().getStringWidth(optStripColor(((ho) chatComponent).g()));
								if (widthCounter > x) {
	...
	private String optStripColor(String text) {
		return bja.a(text, false);
	}

	public boolean isChatOpened() {
		return MinecraftFactory.getVars().getMinecraftScreen() instanceof bkl;
	}
	...
	public int getChatWidth() {
		return ri.d(MinecraftFactory.getClassProxyCallback().get2ndChatWidth());
	}

	public int getChatHeight() {
		return ri.d(this.isChatOpened() ? MinecraftFactory.getClassProxyCallback().get2ndChatHeightFocused() : MinecraftFactory.getClassProxyCallback().get2ndChatHeightUnfocused());
	}
	...
	public static void highlightChatLine(hh chatComponent, int x, int y, int alpha) {
		...
		String builder = "";
		for (hh textComponent : chatComponent) {
			...
			for (String search : highlightWords) {
				...
				for (int nameIndex = builder.toLowerCase(Locale.ROOT).indexOf(search, currIndex), unformattedIndex = text.indexOf(search); nameIndex != -1 && unformattedIndex != -1;
					...
					Gui.drawRect(x + offset, y, x + offset + width, y + MinecraftFactory.getVars().getFontHeight(), 0xffe338 + (Math.min(0x80, alpha) << 24));
				}
			}
		}
	}
