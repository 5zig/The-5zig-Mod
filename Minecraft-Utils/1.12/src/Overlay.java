...
public class Overlay implements IOverlay {
	...
	private static class Toast implements bjz {

		private static final bjz.a ENUM_SHOW, ENUM_HIDE;

		static {
			try {
				Class<?> enumClass = Thread.currentThread().getContextClassLoader().loadClass("bjz" + "$a");
				ENUM_SHOW = (bjz.a) enumClass.getField(Transformer.FORGE ? "SHOW" : "a").get(null);
				ENUM_HIDE = (bjz.a) enumClass.getField(Transformer.FORGE ? "HIDE" : "b").get(null);
			} catch (Exception e) {
		...
		@Override
		public bjz.a a(bka var1, long l) {
			if (updateTime) {
				startedTime = l;
				updateTime = false;
			}
		...
		public static void create(String title, String subTitle, Object uniqueReference) {
			bka toastFactory = ((Variables) MinecraftFactory.getVars()).getMinecraft().ao();
