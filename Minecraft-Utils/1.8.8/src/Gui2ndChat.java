...
//		eu component = new fa("Test");
//		ez style = new ez();
//		style.a(new ew(ew.a.a, new fa("Hover!")));
...
//		((Variables) MinecraftFactory.getVars()).getGuiIngame().d().a(component);
		...
		if (MinecraftFactory.getClassProxyCallback().is2ndChatVisible()) {
			...
			if (chatLineCount > 0) {
				...
				int width = ns.f((float) this.getChatWidth() / chatScale);
				GLUtil.pushMatrix();
				if (Transformer.FORGE) {
						...
						if (ticksPassed < 200 || chatOpened) {
							...
							if (alpha > 3) {
								...
								if (!MinecraftFactory.getClassProxyCallback().isChatBackgroundTransparent()) {
									Gui.drawRect(x, y - 9, x + width + ns.d(4f * chatScale), y, alpha / 2 << 24);
								}
	...
	private void setChatLine(eu chatComponent, int id, int currentUpdateCounter, boolean refresh) {
		if (!refresh && MinecraftFactory.getClassProxyCallback().isShowTimeBeforeChatMessage()) {
			chatComponent = (eu) MinecraftFactory.getClassProxyCallback().getChatComponentWithTime(chatComponent);
		}
		if (id != 0) {
			this.deleteChatLine(id);
		}

		int lineWidth = ns.d((float) this.getChatWidth() / this.getChatScale());
		List<eu> lines = avu.a(chatComponent, lineWidth, ((Variables) MinecraftFactory.getVars()).getFontrenderer(), false, false);
	...
	public void drawComponentHover(int mouseX, int mouseY) {
		...
		if (chatComponent != null && chatComponent.b().i() != null)
			((axu) MinecraftFactory.getVars().getMinecraftScreen()).a(chatComponent, mouseX, mouseY);
	}
	...
	private eu getChatComponent(int mouseX, int mouseY) {
		if (!this.isChatOpened()) {
			...
			avr scaledResolution = new avr(((Variables) MinecraftFactory.getVars()).getMinecraft());
			...
			x = ns.d((float) x / chatScale);
			y = ns.d((float) y / chatScale);
			if (x >= 0 && y >= 0) {
				int lineCount = Math.min(this.getLineCount(), this.singleChatLines.size());
				if (x <= ns.d((float) this.getChatWidth() / this.getChatScale() + 3 / getChatScale()) && y < MinecraftFactory.getVars().getFontHeight() * lineCount) {
					...
					if (lineId >= 0 && lineId < this.singleChatLines.size()) {
						...
						for (eu chatComponent : chatLine.getChatComponent()) {
							if (chatComponent instanceof fa) { // ChatComponentText
								widthCounter += MinecraftFactory.getVars().getStringWidth(optStripColor(((fa) chatComponent).g()));
								if (widthCounter > x) {
	...
	private String optStripColor(String text) {
		return avu.a(text, false);
	}

	public boolean isChatOpened() {
		return MinecraftFactory.getVars().getMinecraftScreen() instanceof awv;
	}
	...
	public int getChatWidth() {
		return ns.d(MinecraftFactory.getClassProxyCallback().get2ndChatWidth());
	}

	public int getChatHeight() {
		return ns.d(this.isChatOpened() ? MinecraftFactory.getClassProxyCallback().get2ndChatHeightFocused() : MinecraftFactory.getClassProxyCallback().get2ndChatHeightUnfocused());
	}
	...
	public static void highlightChatLine(eu chatComponent, int x, int y, int alpha) {
		...
		String builder = "";
		for (eu textComponent : chatComponent) {
