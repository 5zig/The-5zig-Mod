...
public class ServerListHG extends GuiMCPVPServersScroll {

	public ServerListHG(Gui lastScreen) {
		super(lastScreen);
	}

	protected void addServers(Set<MCPVPServer> set) {
		for (MCPVPServer server : set) {
			if ((server.isOnline()) && (server.isAcceptingPlayers()) && (server.getServerType().equals(ServerMCPVP.Server.HARDCORE_GAMES.getIp()))) {
