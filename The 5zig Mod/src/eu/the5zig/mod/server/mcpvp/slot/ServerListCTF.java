...
public class ServerListCTF extends GuiMCPVPServersScroll {

	public ServerListCTF(Gui lastScreen) {
		super(lastScreen);
	}

	protected void addServers(Set<MCPVPServer> set) {
		for (MCPVPServer server : set) {
			if ((server.isOnline()) && (server.isAcceptingPlayers()) && (server.getServerType().equals(ServerMCPVP.Server.CAPTURE_THE_FLAG.getIp()))) {
