...
public class GuiAddFriend extends Gui implements Clickable<GuiAddFriend.ProfileRow> {
	...
	protected void onKeyType(char character, int key) {
		String text = PATTERN.matcher(getTextfieldById(1).getText()).replaceAll("");
		getTextfieldById(1).setText(text);
		if (text.length() >= 3) {
			guiList.setHeader(null);
			guiList.setHeaderPadding(0);
	...
	protected void tick() {
		if (lastTyped != 0 && System.currentTimeMillis() - lastTyped > 500) {
			The5zigMod.getNetworkManager().sendPacket(new PacketUserSearch(PacketUserSearch.Type.KEYWORD, new User[]{new User(getTextfieldById(1).getText(), null)}));
			lastTyped = 0;
		}
	}
