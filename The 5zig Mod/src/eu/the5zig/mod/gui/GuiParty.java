...
public class GuiParty extends Gui {
	...
	public void initGui() {
		...
		if (party != null) {
			...
			getTextfieldById(1).setText(party.getPartyConversation().getCurrentMessage());
	...
	protected void onKeyType(char character, int key) {
		...
		if (key == Keyboard.KEY_UP) {
			textfield.setText(conversation.getPreviousSentMessage());
		}
		if (key == Keyboard.KEY_DOWN) {
			textfield.setText(conversation.getNextSentMessage());
		}
	}
	...
	protected void tick() {
		...
		Party party = The5zigMod.getPartyManager().getParty();
		if (party != null) {
			party.getPartyConversation().setCurrentMessage(getTextfieldById(1).getText());
			getButtonById(100).setEnabled(The5zigMod.getNetworkManager().isConnected() && getTextfieldById(1).getText().length() > 0);
		}
	}
