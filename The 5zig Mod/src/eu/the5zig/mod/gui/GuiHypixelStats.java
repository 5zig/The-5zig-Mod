...
public class GuiHypixelStats extends Gui {
	...
	public void initGui() {
		...
		getTextfieldById(1).setText(player);
	...
	protected void actionPerformed(IButton button) {
		if (button.getId() == 100) {
			if (getTextfieldById(1).getText().isEmpty())
				return;
			refresh(player = getTextfieldById(1).getText());
		}
	}
	...
	protected void drawScreen(int mouseX, int mouseY, float partialTicks) {
		if (status == null) {
			guiListGameTypes.drawScreen(mouseX, mouseY, partialTicks);
			guiListStat.drawScreen(mouseX, mouseY, partialTicks);
	...
	protected void handleMouseInput() {
		if (status == null) {
			guiListGameTypes.handleMouseInput();
			guiListStat.handleMouseInput();
		}
	}
	...
	protected void onKeyType(char character, int key) {
		ITextfield textfield = getTextfieldById(1);
		if (textfield.isFocused() && !The5zigMod.getVars().isPlayerNull() && key == Keyboard.KEY_TAB) {
			if (tabWord == null) {
				tabWord = textfield.getText().toLowerCase();
			}
			...
			if (!tabPlayers.isEmpty()) {
				...
				textfield.setText(playerInfo.getGameProfile().getName());
