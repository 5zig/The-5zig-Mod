			...
			synchronized (chatLines) {
				chatLines.add(new ViewMoreRow(100 + getChatBoxWidth() / 2, new Callback<IButton>() {
					...
					public void call(IButton button) {
						...
						final float currentScroll = guiList.getContentHeight() - guiList.getCurrentScroll(); // store the current scroll. Normally, somewhere around 0.0.
...
//								guiList.calculateHeightMap(); // calculate heightmap.
	...
	public void handleMouseInput() {
		conversationList.handleMouseInput();
		chatList.handleMouseInput();
	}
	...
	protected void tick() {
		...
		searchText = getTextfieldById(9991).getText();
	...
	private void enableDisableButtons() {
		...
		sendButton.setEnabled(The5zigMod.getNetworkManager().isConnected() && textfield.getText().length() > 0 && getSelectedConversation() != null &&
