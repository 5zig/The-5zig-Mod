...
public class GuiEditChatMessage extends Gui {
	...
	public void initGui() {
		...
		if (chatMessage.getName() != null) {
			getTextfieldById(9).setText(chatMessage.getName());
		}
		...
		getTextfieldById(1).setText(chatMessage.getMessage());
		getTextfieldById(3).setText(chatMessage.getExcept() == null ? "" : chatMessage.getExcept());
		...
		if (chatMessage.getAction() == ChatFilter.Action.AUTO_TEXT) {
			...
			if (chatMessage.getAutoText() != null) {
				getTextfieldById(4).setText(chatMessage.getAutoText());
			}
			getTextfieldById(5).setText(String.valueOf(chatMessage.getAutoTextDelay()));
			if (chatMessage.getAutoTextCancel() != null) {
				getTextfieldById(6).setText(chatMessage.getAutoTextCancel());
			}
		}
	...
	protected void actionPerformed(IButton button) {
		if (button.getId() == 200) {
			if (!getTextfieldById(9).getText().isEmpty()) {
				chatMessage.setName(getTextfieldById(9).getText());
			} else {
				chatMessage.setName(null);
			}
			chatMessage.setUseRegex(chatMessageCopy.useRegex());
			chatMessage.setMessage(getTextfieldById(1).getText());
			chatMessage.setExcept(getTextfieldById(3).getText());
			...
			if (getTextfieldById(4) != null && !getTextfieldById(4).getText().isEmpty()) {
				chatMessage.setAutoText(getTextfieldById(4).getText());
			}
			if (getTextfieldById(5) != null && !getTextfieldById(5).getText().isEmpty()) {
				try {
					int delay = Integer.parseInt(getTextfieldById(5).getText());
					if (delay > 0) {
			...
			if (getTextfieldById(6) != null && !getTextfieldById(6).getText().isEmpty()) {
				chatMessage.setAutoTextCancel(getTextfieldById(6).getText());
			} else {
	...
	protected void mouseReleased(int x, int y, int state) {
		if (displayHelp) {
			closeHelpButton.mouseReleased(x, y);
		}
	}
