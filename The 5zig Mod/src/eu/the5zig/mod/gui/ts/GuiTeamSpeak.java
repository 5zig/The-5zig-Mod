...
//		});
	...
	protected void actionPerformed(IButton button) {
		if (button.getId() == 100) {
			...
			if (textfield.getText().length() <= 1024 && chatTabView.selectedElement != null && chatTabView.selectedElement.getType() != MessageTargetMode.POKE) {
				chatTabView.selectedElement.sendMessage(textfield.getText());
				textfield.setText("");
			}
		}
		if (button.getId() == 50) {
	...
	protected void tick() {
		...
		if (serverTab != null) {
			...
			if (serverTab.getServerInfo() != null) {
				...
				int previousContentHeight = chatList.getContentHeight();
