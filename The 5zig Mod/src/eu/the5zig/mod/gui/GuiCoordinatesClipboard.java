...
public class GuiCoordinatesClipboard extends GuiOptions {
	...
	public void initGui() {
		...
		if (location != null) {
			getTextfieldById(1).setText(String.valueOf(location.getX()));
			getTextfieldById(2).setText(String.valueOf(location.getY()));
		}
	...
	protected void actionPerformed(IButton button) {
		if (button.getId() == 1) {
			...
			if (getTextfieldById(1).getText().isEmpty() && getTextfieldById(2).getText().isEmpty()) {
		...
		if (button.getId() == 2) {
			...
			if (next == null) {
				...
				getTextfieldById(1).setText(String.valueOf(next.getLocation().getX()));
				getTextfieldById(2).setText(String.valueOf(next.getLocation().getY()));
			}
	...
	private void trySetLocation() {
		if (getTextfieldById(1).getText().length() > 0 && getTextfieldById(2).getText().length() > 0) {
			try {
				Vector2i location = new Vector2i(Integer.parseInt(getTextfieldById(1).getText()), Integer.parseInt(getTextfieldById(2).getText()));
	...
	protected void onKeyType(char character, int key) {
		if (key == Keyboard.KEY_SPACE && !The5zigMod.getVars().isPlayerNull()) {
			getTextfieldById(1).setText(String.valueOf((int) Math.floor(The5zigMod.getVars().getPlayerPosX())));
			getTextfieldById(2).setText(String.valueOf((int) Math.floor(The5zigMod.getVars().getPlayerPosZ())));
		}
		...
		if (key == Keyboard.KEY_D && The5zigMod.isCtrlKeyDown() && deathLocation != null) {
			getTextfieldById(1).setText(String.valueOf((int) Math.floor(deathLocation.getCoordinates().getX())));
			getTextfieldById(2).setText(String.valueOf((int) Math.floor(deathLocation.getCoordinates().getZ())));
		}
	}
	...
	protected void tick() {
		...
		boolean enabled = textfields.get(0).getText().length() > 0 && textfields.get(1).getText().length() > 0 && isInt(textfields.get(0).getText()) &&
				isInt(textfields.get(1).getText());
		getButtonById(2).setEnabled(enabled);
		getButtonById(100).setEnabled(enabled || (textfields.get(0).getText().length() == 0 && textfields.get(1).getText().length() == 0));
